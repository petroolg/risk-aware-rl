...
class SGDRegressor_occupancy:
    def __init__(self, xd, restore=True):
        ...
        self.smce = tf.losses.mean_squared_error(tf.gather_nd(self.actions, self.action_ind), self.pi, weights=self.weights_picked)
    ...
    def partial_fit(self, sa_pairs, actions):
        ...
        self.session.run(self.train, feed_dict={self.actions:actions, self.states:sa_pairs[:,:-2],self.action_ind:inds, self.weight_ind:weight_inds})
    ...
    def predict(self, sa_pairs):
        ...
        weight_inds = np.vstack((np.zeros((1, len(sa_pairs))), weight_inds))
        weight_inds = weight_inds.T
        return self.session.run(self.pi, feed_dict={self.states:sa_pairs[:,:-2], self.action_ind:inds, self.weight_ind:weight_inds})
    ...
    def look_incide(self,sa_pairs, actions):
        ...
        weights =  self.session.run(self.weights_picked, feed_dict={self.states: sa_pairs[:,:-2],self.weight_ind:weight_inds})
        softm = self.session.run(self.smce, feed_dict={self.actions:actions, self.states:sa_pairs[:,:-2],self.action_ind:inds,self.weight_ind:weight_inds})
        c0 = self.session.run(self.cost, feed_dict={self.actions:actions, self.states:sa_pairs[:,:-2], self.action_ind:inds,self.weight_ind:weight_inds})
